...
import {
  ...
  validateField,
} from "../../modules/validation/validator";
...
export class FFormBuilder extends FRoot {
    ...
    if (ref.value) {
      ...
      const fieldValidation = async (event: Event) => {
        ...
        if (this.values && this.field && this.field.type === "array") {
          ...
          Object.assign(
            this.values,
            (ref.value as FFormInputElements).value as Object,
          );
        } else {
  ...
  dispatchStateChangeEvent() {
    if (
      (this.lastState && !isEqual(this.lastState, this.state)) ||
